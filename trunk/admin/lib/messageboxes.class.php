...
/* Notitz: Hinzufügen und ähnliches geht in etwa so:
- Es wird ne Funktion Add-entry oder add-comment aufgerufen
- Die Funktion prüft die Korrektheit, und gibt bei Erfolg den Fehlercode 0 aus, und speichert die Daten.
- Bei Fehler gibt sie einen Fehlercode aus, anhand der gefunden Fehler.
- Die Module fragen anschliessend zurück, welche Fehler es gibt (show_failer).
- Das Modul gibt die Fehler zurück.

Ungefähre Idee, neuster Stand, nix so wie ist:
Klasse bekommt entweder ein Array, oder die anzahl der zurückgegebenen werten.
Erstellen des Array's mit den werten, oder überprüfen anhand bekannter kriterien (default, email, webpage).

Problem: Woher weiss die Klasse, welcher Datensatz des Arrays zu welcher Zelle in der DB-Tabelle gehört?
Wie überprüfe ich, ob alle nötigen werte, die von der MySQL-Tabelle nötig sind, angegeben wurden?
...
/**
 * Klassenkonstruktor
 * Alle Variablen ausser des Tabellennamens sind optional, d.H werden erst für einen Comment oder
 ...
 * @param int $navigation_id ID der Applikation (z.B. Navigations-ID des Gästebuchs)
