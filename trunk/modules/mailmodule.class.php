...
class Mailmodule implements Module
{
	...
	{
		$this->_mysql->query("SELECT `modules`.`modules_ID`, `modules`.`modules_mail_support` FROM `modules`, `menu` WHERE
		 `menu`.`menu_ID` = '$mod_navID' AND `menu`.`menu_pagetyp` = 'mod' AND 
		 `menu`.`menu_page` = `modules`.`modules_ID` LIMIT 1");
		...
		$this->_mysql->query("SELECT `modules_mail_table` as 'table', `modules_mail_columns` as 'columns' FROM `modules` WHERE `modules_ID` = '{$mod_data[0]}' LIMIT 1");
